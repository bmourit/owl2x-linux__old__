...
int vivante_hw_init(struct vivante_gpu *gpu)
{
	...
	gpu->id = vivante_register_mmu(gpu->dev, gpu->mmu);
		...
		const char *ioname, const char *irqname)
{
	...
	ret = vivante_gpu_init(dev, gpu, &funcs, name, ioname, irqname);
	if (ret < 0) {
...
static void hangcheck_timer_reset(struct vivante_gpu *gpu)
{
	...
	mod_timer(&gpu->hangcheck_timer,
			round_jiffies_up(jiffies + DRM_VIVANTE_HANGCHECK_JIFFIES));
}
		...
		struct vivante_file_private *ctx)
{
	...
	for (i = 0; i < submit->nr_bos; i++) {
		...
		if (!is_active(vivante_obj)) {
			...
			vivante_gem_get_iova_locked(&vivante_obj->base,
					submit->gpu->id, &iova);
		}

		if (submit->bos[i].flags & VIVANTE_SUBMIT_BO_READ)
			vivante_gem_move_to_active(&vivante_obj->base, gpu, false, submit->fence);

		if (submit->bos[i].flags & VIVANTE_SUBMIT_BO_WRITE)
...
int vivante_gpu_init(struct drm_device *drm,struct vivante_gpu *gpu,
...
void vivante_gpu_cleanup(struct vivante_gpu *gpu)
{
	DBG("%s", gpu->name);
