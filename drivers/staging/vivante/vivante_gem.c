...
static struct page **get_pages(struct drm_gem_object *obj)
{
	...
	if (!vivante_obj->pages) {
		 ...
		 */
		if (vivante_obj->flags & (VIVANTE_BO_WC|VIVANTE_BO_UNCACHED))
...
int vivante_gem_mmap_obj(struct drm_gem_object *obj,
		struct vm_area_struct *vma)
{
	...
	if (vivante_obj->flags & VIVANTE_BO_WC) {
		vma->vm_page_prot = pgprot_writecombine(vm_get_page_prot(vma->vm_flags));
	} else if (vivante_obj->flags & VIVANTE_BO_UNCACHED) {
		...
		struct drm_mode_create_dumb *args)
{
	...
	return vivante_gem_new_handle(dev, file, args->size,
			VIVANTE_BO_SCANOUT | VIVANTE_BO_WC, &args->handle);
}
		...
		uint32_t handle, uint64_t *offset)
{
	...
	*offset = vivante_gem_mmap_offset(obj);
		...
		struct timespec *timeout)
{
	...
	if (is_active(vivante_obj)) {
		...
		if (op & VIVANTE_PREP_READ)
			fence = vivante_obj->write_fence;
		if (op & VIVANTE_PREP_WRITE)
			fence = max(fence, vivante_obj->read_fence);
		if (op & VIVANTE_PREP_NOSYNC)
			timeout = NULL;
		...
		struct drm_gem_object **obj)
{
	...
	switch (flags & VIVANTE_BO_CACHE_MASK) {
	case VIVANTE_BO_UNCACHED:
	case VIVANTE_BO_CACHED:
	case VIVANTE_BO_WC:
		break;
	default:
		dev_err(dev->dev, "invalid cache flag: %x\n",
				(flags & VIVANTE_BO_CACHE_MASK));
		return -EINVAL;
	}
		...
		uint32_t size, struct sg_table *sgt)
{
	...
	ret = vivante_gem_new_impl(dev, size, VIVANTE_BO_WC, &obj);
	if (ret)
		goto fail;
