...
static void dwc3_clk_init(struct dwc3_actions *actions)
{
	u32		reg;
	struct dwc3_port_info *port_info = &actions->port_info;
	...
	/*PLL1 enable*/
	reg = readl(actions->base + DWC3_CMU_DEBUG_LDO);
	reg |= CMU_BIAS_EN;
	writel(reg, actions->base + DWC3_CMU_DEBUG_LDO);
	...
	writel(reg, actions->base + DWC3_CMU_PLL2_BISTDEBUG);
...
static void dwc3_clk_exit(struct dwc3_actions *actions)
{
	u32		reg;
	struct dwc3_port_info *port_info = &actions->port_info;
...
static int __devinit dwc3_actions_probe(struct platform_device *pdev)
{
	...
	actions = devm_kzalloc(dev,sizeof(*actions), GFP_KERNEL);
	if (!actions) {
		dev_err(&pdev->dev, "not enough memory\n");
		goto err1;
	}

	platform_set_drvdata(pdev, actions);
	...
	actions->port_info.devrst = (void __iomem *)IO_ADDRESS(CMU_DEVRST1);
	actions->port_info.usbecs = (void __iomem *)IO_ADDRESS(USB3_P0_CTL);
	actions->port_info.usbpll = (void __iomem *)IO_ADDRESS(CMU_USBCLK);
	...
	actions->dwc3	= dwc3;
	actions->dev	= &pdev->dev;
	actions->base	= base;

	dwc3_clk_init(actions);
...
#ifdef CONFIG_PM
static int dwc3_actions_suspend(struct platform_device *pdev, pm_mess1age_t state)
{
	struct dwc3_actions	*actions = platform_get_drvdata(pdev);
...
#endif
    dwc3_clk_exit(actions);
	platform_device_unregister(actions->dwc3);
	dwc3_put_device_id(actions->dwc3->id);
