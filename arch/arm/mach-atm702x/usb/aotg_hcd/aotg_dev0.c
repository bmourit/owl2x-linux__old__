...
//#define AOTG0_MAJOR	 	222
//#define AOTG0_MINOR     	0
...
static struct aotg_plat_data otg_data0 = {
...
static struct resource hcd_res0[] = {
	[0] = {
		.start = AOTG0_BASE,
...
static u64 hcd_dmamask0 = ~(u32) 0;

static struct platform_device *aotg_dev0 = NULL;
...
int aotg0_device_init(int power_only)
{
	...
	aotg_dev0->dev.dma_mask = &hcd_dmamask0,
	aotg_dev0->dev.coherent_dma_mask = 0xffffffff,

	ret = platform_device_add_resources(aotg_dev0, hcd_res0,
			ARRAY_SIZE(hcd_res0));
	if (ret) {
		printk("couldn't add resources to aotg_dev0 device\n");
		goto err2;
	}

	ret = platform_device_add_data(aotg_dev0, &otg_data0, sizeof(otg_data0));
	if (ret) {
		printk("couldn't add data to aotg_dev0 device\n");
		goto err2;
	}

	ret = platform_device_add(aotg_dev0);
	if (ret) {
		printk("failed to register aotg0 device\n");
		goto err2;
	}
...
void aotg0_device_mod_init(void)
{
	...
	printk("usb0_gpio_vbus_pin get:%d\n", usb_gpio_vbus_pin);
