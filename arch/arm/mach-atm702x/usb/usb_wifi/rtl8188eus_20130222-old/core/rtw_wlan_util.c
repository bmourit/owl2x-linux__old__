...
u8 rtw_dump_frame(u8 *pframe, u32 packet_len)
{
	u8 					*ptr = pframe;
	int					i, n;
...
//phchen
#define BEACON_OFFSET 			24
#define BEACON_INFO_OFFSET		36
int rtw_check_beacon_package(u8 *pframe, u32 packet_len)
{
	u8 u8len;
	u8 *pt = pframe;
	u32 plen, u32len;
	
	if (packet_len <= (BEACON_INFO_OFFSET + 2))
	{
		/* Is not beacon package */
		return -1;
	}
	
	plen = BEACON_INFO_OFFSET;
	pt +=  plen ;
...
int rtw_check_bcn_info(ADAPTER *Adapter, u8 *pframe, u32 packet_len)
{
	...
	if (cur_network->BcnInfo.encryp_protocol != encryp_protocol) {
		...
		// phchen
		if (rtw_check_beacon_package( pframe,  packet_len) == 0)
			goto _mismatch;
