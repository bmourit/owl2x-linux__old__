...
//phchen
u8 rtw_dump_recv_frame(union recv_frame *precv_frame)
{
	struct recv_frame_hdr *precv_hdr = &precv_frame->u.hdr;
	u8 					*ptr = precv_frame->u.hdr.rx_data;
	int					i, n;

	DBG_871X("rtw_dump_recv_frame: dump packet \n");
	
	
	*ptr = precv_frame->u.hdr.rx_data;
	n = precv_frame->u.hdr.len;
	for(i=0;i<n;i++)
	{
		DBG_871X("%02X  ",ptr[i]);
		if((i+1) %8 ==0)
...
//phchen
extern int rtw_check_beacon_package(u8 *pframe, u32 packet_len);
		...
		if(((pmlmeinfo->state&0x03) == WIFI_FW_STATION_STATE) && (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS))
		{
			if ((psta = rtw_get_stainfo(pstapriv, GetAddr2Ptr(pframe))) != NULL)
			{
				...
				if (!ret) {
						...
						//phchen
						rtw_dump_recv_frame(precv_frame);
				...
				if (ret == _IGNORE)
				{
					/* ignore the beacon packet */
					DBG_871X_LEVEL(_drv_always_, "Ignore the beacon packet\n ");
					return _SUCCESS;
				}
