...
VOID MlmeNewTxRate(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	if (ADAPT_RATE_TABLE(pTable))
		...
		if( pNextTxRate == NULL )
		{
        	DBGPRINT_RAW(RT_DEBUG_ERROR, ("pNextTxRate is set as invalid 1111, return, pEntry=%p\n", pEntry));
            if (pEntry)
            {
            	pEntry->FailRateCount++;
				DBGPRINT_RAW(RT_DEBUG_ERROR,("pNextTxRate is set as invalid 2222, return, pEntry->FailRateCount=%u \n", pEntry-> FailRateCount));
			}
            return;
         }
	/*  Set new rate */
