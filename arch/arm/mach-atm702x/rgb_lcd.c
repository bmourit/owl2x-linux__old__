...
int rgb_lcd_init(struct asoc_display_device * disp_dev)
{
	unsigned int tmp = 0;
	...
	tmp = tmp|(data_width << 16) | (0x1 << 6) | 0x1 | (0x0 << 10) | (0x0 << 13); //rgb if, data width, video from de,enable lcdc
...
int rgb_lcd_close(struct asoc_display_device * disp_dev)
{
	int tmp;
	int LCD_REG_BASE = 0;
	int lcd_id =  disp_dev->display_id;
...
int rgb_lcd_get_pin(struct asoc_display_device * disp_dev)
{
	struct lcd_manager *lcd_mgr = disp_dev_get_drvdata(disp_dev);
	
    act_writel(act_readl(PAD_CTL) | 0x02,PAD_CTL);
	//act_writel((act_readl(MFP_CTL1)  | 0x66600000),MFP_CTL1);
...
int rgb_lcd_release_pin(struct asoc_display_device * disp_dev){
	struct lcd_manager *lcd_mgr = disp_dev_get_drvdata(disp_dev);
...
int rgb_lcd_get_mdsb_parameter(struct asoc_display_device *disp_dev,
...
int lcd_manager_init(struct lcd_manager * lcd_mgr){
	...
	lcd_mgr->lcd_init = rgb_lcd_init;
	lcd_mgr->lcd_close = rgb_lcd_close;
	lcd_mgr->get_pin = rgb_lcd_get_pin;
	lcd_mgr->release_pin = rgb_lcd_release_pin;
	lcd_mgr->get_mdsb_parameter = rgb_lcd_get_mdsb_parameter;
	return 0;
}